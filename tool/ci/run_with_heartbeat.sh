...
# session leader because util-linux `setsid --wait` may fork. BusyBox and older
# implementations reject `--wait`, so probe and use plain `setsid` only when
# it remains the session leader whose status this wrapper can observe.
