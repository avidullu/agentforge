...
shift
if [[ ${1:-} == true ]]; then
  exec "$AGENTFORGE_REAL_SETSID" --wait true
fi
...
# BusyBox/older setsid rejects --wait. Plain setsid is accepted only when it
# remains the observable session leader and preserves the command status.
plain_bin=$temp_dir/plain-bin
mkdir "$plain_bin"
cat >"$plain_bin/setsid" <<'PLAIN_SETSID'
#!/usr/bin/env bash
set -Eeuo pipefail
if [[ ${1:-} == --wait ]]; then
  exit 64
fi
exec "$AGENTFORGE_REAL_SETSID" "$@"
PLAIN_SETSID
chmod +x "$plain_bin/setsid"
status=0
PATH="$plain_bin:$PATH" AGENTFORGE_REAL_SETSID="$real_setsid" \
  bash "$script_dir/run_with_heartbeat.sh" plain-setsid-failure 1 -- \
  bash -c 'exit 7' >"$temp_dir/plain-setsid.log" 2>&1 || status=$?
cat "$temp_dir/plain-setsid.log"
[[ $status -eq 7 ]]
grep -Fq 'WARN setsid --wait unavailable; using plain setsid' \
  "$temp_dir/plain-setsid.log"
